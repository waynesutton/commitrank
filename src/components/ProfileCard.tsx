...
import { Twitter, Github, Globe, MapPin, Sun, Moon, Share2, Zap, Code2, Wand2, Sword, Sprout, Compass, Database } from 'lucide-react';
...
export default function ProfileCard({ profile, commits, usesConvex }: ProfileCardProps) {
  ...
  const shareOnTwitter = () => {
    ...
    const text = `🎮 Just discovered ${profile.name} is a ${rankLabel} on Commit Quest with ${commits.toLocaleString()} commits! ${usesConvex ? '⚡ Convex Developer!' : ''} Check out their journey at https://commitquest.dev #CommitQuest #GitHubGaming`;
  ...
  return (
    <div className={`rounded-lg shadow-xl p-6 w-[450px] mx-auto ${cardClass} transition-colors duration-300`}>
      ...
      <div className="mt-6 space-y-2">
        <p className="text-xl font-bold">
          {commits.toLocaleString()} Total Commits
        </p>
      ...
      <div className="mt-6 space-y-2">
        ...
        {profile.twitter_username && (
          <div className="flex items-center gap-2">
            <Twitter size={16} />
            <a href={`https://twitter.com/${profile.twitter_username}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
              @{profile.twitter_username}
            </a>
          </div>
        )}
        <div className="flex items-center gap-2">
          <Github size={16} />
          <a href={profile.html_url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
            GitHub Profile
          </a>
        </div>
      </div>
